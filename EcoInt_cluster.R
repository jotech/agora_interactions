...
#agora <- readRDS(paste0("/home/swaschina/workspace-kiel/Resources/AGORA/AGORA/",args[1],".RDS"))
agora <- readRDS(paste0(args[1],".RDS"))
if(args[2] == "SWcorr") {
...
cat(system.time(DFinteract <- foreach(i=1:ncol(comb)) %dopar% {
  if(i %% (nr.cores*15-1) == 0) writeProcessToFile(ts,as.numeric(sub(paste0(" ",log.file),"",
...
saveRDS(DFgrowth, file = paste0("results/DFgrowth_",args[1],"_",args[2],"_",args[3],".RDS"))
saveRDS(DFmets, file = paste0("results/DFmets_",args[1],"_",args[2],"_",args[3],".RDS"))
